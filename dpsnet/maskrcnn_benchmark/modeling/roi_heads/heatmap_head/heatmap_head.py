...
class HeatmapHead(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.heatmap_convs = nn.Sequential(
            ...
            nn.InstanceNorm2d(self.num_classes,  affine=True),
            #nn.BatchNorm2d(self.num_classes),
            nn.ReLU(inplace=True),
            #nn.Sigmoid(), if change, there would be error like 
        )
        ...
        self.density_criterion = nn.SmoothL1Loss(reduction='mean')
    ...
    def compat_forward(self, features, detections, targets=None):
        ...
        ## when generate density map, the kenerl value multiplied 100,
        ## so there has to divied 100
        if not self.training:
            density = torch.sum(density_logits/100.0, dim=(2, 3))  # (batch, num_classes)
            for i, detection in enumerate(detections):
                detection.add_field('density', density[i])
                # temp add
