...
class Heatmap(object):
    ...
    def __init__(self, heatmap, count=0):
        # (num_classes, height, width)
        assert isinstance(heatmap, torch.Tensor) and heatmap.ndimension() == 3
    ...
    def transpose(self, method):
        ...
        if method == FLIP_LEFT_RIGHT:
            return Heatmap(torch.flip(self.heatmap, dims=(2,)), count=self.count)
        elif method == FLIP_TOP_BOTTOM:
            return Heatmap(torch.flip(self.heatmap, dims=(1,)), count=self.count)

    def to(self, *args, **kwargs):
        heatmap = self.heatmap.to(*args, **kwargs)
        return Heatmap(heatmap, count=self.count)
